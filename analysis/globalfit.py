...
#        print ' theres.tsys =  ', theres.tsys
    ...
    resdict[(delt,delp)] = [a_tsys, deltat, a_errtsys, a_errortofmax]
...
################################
#outname = 
def getres(theta,phi,outn):
    resfile = open(outn,'rb')
    resdict = pickle.load(resfile)
    res = resdict[(theta,phi)]
    print 'resdict = ' , res

def chi2func(t,tsys,deltat,errtsys,errtofmax):
    return np.sum( (tsys - t)**2/(tsys*errtsys)**2 ) + np.sum( deltat**2 / errtofmax**2)
#    return np.sum( deltat**2 / errtofmax**2)

tsysspace = np.linspace(20,120,10)
...
for s in listofangle:
    res = resdict[s]
    for t in tsysspace:
        chi2 = chi2func(t, res[0], res[1], res[2], res[3])
#        print 's = ', s, 'chi2 = ' , chi2
        if chi2 < minchi2:
            minchi2 = chi2
            thetamin = s[0]
            phimin = s[1]
            tmin = t
print 'chi2min = ' , minchi2 , ' thetami =  ' , thetamin, ' phimin = ' , phimin , 'tmin = '  , tmin
